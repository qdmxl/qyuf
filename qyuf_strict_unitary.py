...
class FeatureFractal:
    ...
    # ─────────────────────────────────────────────────────────
    # 高维语义编码 (v4.1+): 64维直接编码替代6维物理特征
    # ─────────────────────────────────────────────────────────
    
    # 八卦名称列表
    _BAGUA_NAMES = ["乾", "兑", "离", "震", "巽", "坎", "艮", "坤"]
    
    # 64卦基底的语义标签: 64个关键词/概念
    _HEX_SEMANTIC = {
        # (上卦, 下卦) → 编码贡献
        # 这里不做硬编码，而是通过语义规则动态生成
    }
    
    # 语义关键词→64卦映射表（每个关键词激活一组卦）
    _SEMANTIC_HEX_MAP = {
        # 容器类
        "cabinet":   [(5,4, 1.0), (5,5, 0.8), (5,6, 0.6)],  # 柜→巽巽/巽艮/巽坎
        "drawer":    [(5,6, 1.0), (5,5, 0.7), (6,6, 0.5)],  # 抽屉→巽艮/巽巽/艮艮
        "shelf":     [(6,4, 1.0), (6,5, 0.8), (4,5, 0.5)],  # 架→艮巽/艮巽/震巽
        "desk":      [(6,6, 1.0), (6,7, 0.7), (7,6, 0.6)],  # 桌→艮艮/艮坤/坤艮
        "table":     [(6,6, 1.0), (6,7, 0.7), (7,6, 0.6)],
        "counter":   [(6,6, 0.8), (7,6, 0.8), (6,5, 0.6)],
        "bed":       [(4,0, 1.0), (4,1, 0.7), (0,4, 0.6)],  # 床→震乾/震兑/乾震
        "sofa":      [(0,1, 1.0), (0,0, 0.7), (1,0, 0.6)],  # 沙发→乾兑/乾坤/兑乾? 用坤
        "chair":     [(1,4, 1.0), (1,1, 0.7), (4,1, 0.5)],  # 椅→离震/离兑/震离
        "fridge":    [(2,2, 1.0), (2,3, 0.8), (3,2, 0.6)],  # 冰箱→坎坎/坎巽/巽坎
        "oven":      [(3,3, 1.0), (3,2, 0.7), (3,4, 0.6)],  # 炉→巽巽/巽坎/巽震? 火→离
        "stove":     [(3,3, 1.0), (3,4, 0.8), (4,3, 0.6)],
        "sink":      [(2,3, 1.0), (2,2, 0.8), (3,2, 0.6)],  # 水槽→坎巽/坎坎/巽坎
        "toilet":    [(2,1, 1.0), (2,0, 0.7), (1,2, 0.6)],  # 马桶→坎兑/坎乾/兑坎
        "bin":       [(1,7, 1.0), (7,1, 0.8), (1,1, 0.5)],  # 桶→兑坤/坤兑/兑兑
        "garbage":   [(1,7, 1.0), (7,1, 0.8), (7,7, 0.6)],
        "trash":     [(1,7, 1.0), (7,1, 0.8), (7,7, 0.6)],
        "lamp":      [(4,4, 1.0), (4,3, 0.8), (3,4, 0.7)],  # 灯→震震/震巽/巽震
        "tv":        [(4,1, 1.0), (4,0, 0.7), (0,4, 0.5)],  # 电视→震兑/震乾/乾震
        "microwave": [(4,3, 1.0), (3,3, 0.8), (4,4, 0.6)],  # 微波→震巽/巽巽/震震
        "dresser":   [(6,6, 1.0), (6,7, 0.8), (7,6, 0.6)],
        "coffee":    [(5,3, 1.0), (5,5, 0.7), (3,5, 0.6)],  # 咖啡→巽巽/巽巽? 用离
        "nightstand":[(7,7, 1.0), (7,6, 0.8), (6,7, 0.7)],
        "safe":      [(5,6, 1.0), (6,5, 0.8), (6,6, 0.6)],  # 保险柜→巽艮/艮巽/艮艮
        "rug":       [(7,7, 1.0), (7,0, 0.7), (0,7, 0.6)],  # 地毯→坤坤/坤乾/乾坤
        "laundry":   [(2,7, 1.0), (7,2, 0.7), (2,2, 0.5)],  # 洗衣→坎坤/坤坎/坎坎
        
        # 物体类
        "apple":     [(7,4, 1.0), (7,7, 0.7), (4,7, 0.6)],  # 苹果→坤震/坤坤/震坤
        "tomato":    [(7,4, 1.0), (4,7, 0.8), (7,7, 0.5)],
        "egg":       [(7,4, 0.9), (4,7, 0.7), (7,1, 0.6)],  # 蛋→坤震/震坤/坤兑
        "potato":    [(7,6, 1.0), (6,7, 0.7), (7,7, 0.6)],
        "bread":     [(7,7, 1.0), (7,1, 0.7), (7,5, 0.6)],
        "wine":      [(2,2, 1.0), (2,3, 0.7), (3,2, 0.6)],  # 酒→坎坎/坎巽/巽坎
        "plate":     [(6,5, 1.0), (5,6, 0.8), (6,6, 0.6)],  # 盘→艮巽/巽艮/艮艮
        "bowl":      [(2,5, 1.0), (5,2, 0.8), (2,2, 0.6)],  # 碗→坎巽/巽坎/坎坎
        "cup":       [(2,1, 1.0), (1,2, 0.7), (2,2, 0.5)],  # 杯→坎兑/兑坎/坎坎
        "spoon":     [(1,1, 1.0), (1,0, 0.7), (0,1, 0.6)],  # 勺→兑兑/兑乾/乾兑
        "fork":      [(1,0, 1.0), (0,1, 0.7), (1,1, 0.6)],  # 叉→兑乾/乾兑/兑兑
        "knife":     [(0,1, 1.0), (1,0, 0.8), (0,0, 0.6)],  # 刀→乾兑/兑乾/乾坤
        "pan":       [(3,4, 1.0), (4,3, 0.7), (3,3, 0.6)],  # 锅→巽震/震巽/巽巽
        "book":      [(5,5, 1.0), (5,7, 0.7), (7,5, 0.6)],  # 书→巽巽/巽坤/坤巽
        "remote":    [(1,4, 1.0), (4,1, 0.7), (1,3, 0.6)],  # 遥控器→兑震/震兑/兑巽
        "pillow":    [(7,2, 1.0), (7,7, 0.8), (2,7, 0.6)],  # 枕头→坤坎/坤坤/坎坤
        "towel":     [(2,4, 1.0), (4,2, 0.7), (2,7, 0.6)],  # 毛巾→坎震/震坎/坎坤
        "clock":     [(0,0, 1.0), (0,4, 0.7), (4,0, 0.6)],  # 钟→乾坤/乾震/震乾
        "statue":    [(0,6, 1.0), (6,0, 0.7), (0,0, 0.6)],  # 像→乾艮/艮乾/乾坤
        "candle":    [(4,3, 1.0), (3,4, 0.7), (4,4, 0.6)],  # 蜡烛→震巽/巽震/震震
        "card":      [(5,5, 1.0), (5,7, 0.7), (7,5, 0.6)],  # 卡→巽巽/巽坤/坤巽
        "soap":      [(2,5, 1.0), (5,2, 0.7), (2,2, 0.6)],  # 肥皂→坎巽/巽坎/坎坎
        "sponge":    [(2,2, 1.0), (2,3, 0.7), (3,2, 0.6)],  # 海绵→坎坎/坎巽/巽坎
        "lettuce":   [(7,3, 1.0), (7,4, 0.7), (4,7, 0.5)],  # 生菜→坤巽/坤震/震坤
        "spray":     [(2,4, 1.0), (4,2, 0.8), (2,2, 0.5)],  # 喷雾→坎震/震坎/坎坎
        
        # 动词类
        "go":        [(0,4, 1.0), (4,0, 0.8), (0,0, 0.6)],  # 去→乾震/震乾/乾坤
        "take":      [(4,0, 1.0), (0,4, 0.7), (4,4, 0.5)],  # 拿→震乾/乾震/震震
        "put":       [(7,7, 1.0), (7,6, 0.7), (6,7, 0.6)],  # 放→坤坤/坤艮/艮坤
        "open":      [(0,0, 1.0), (0,1, 0.8), (1,0, 0.6)],  # 开→乾坤/乾兑/兑乾
        "close":     [(6,6, 1.0), (6,7, 0.7), (7,6, 0.6)],  # 关→艮艮/艮坤/坤艮
        "heat":      [(4,3, 1.0), (3,4, 0.8), (4,4, 0.6)],  # 加热→震巽/巽震/震震
        "cool":      [(2,2, 1.0), (2,3, 0.7), (3,2, 0.6)],  # 冷却→坎坎/坎巽/巽坎
        "clean":     [(2,4, 1.0), (4,2, 0.7), (2,2, 0.6)],  # 洗→坎震/震坎/坎坎
        "examine":   [(0,5, 1.0), (5,0, 0.7), (0,0, 0.6)],  # 看→乾巽/巽乾/乾坤
        "use":       [(4,0, 1.0), (0,4, 0.7), (4,4, 0.6)],  # 用→震乾/乾震/震震
        "look":      [(0,5, 1.0), (5,0, 0.8), (0,0, 0.5)],  # 看→乾巽/巽乾/乾坤
        "inventory": [(7,5, 1.0), (5,7, 0.7), (7,7, 0.6)],  # 查→坤巽/巽坤/坤坤
        "help":      [(5,7, 1.0), (7,5, 0.7), (5,5, 0.6)],  # 帮助→巽坤/坤巽/巽巽
    }
    
    _KEYWORD_CACHE = {}  # 名称→卦编码缓存
    
    @classmethod
    def encode_semantic_64(cls, name: str) -> np.ndarray:
        """
        高维语义编码: 实体名称→64卦概率分布
        
        通过语义关键词匹配，直接编码到64卦空间。
        这是替代6维物理特征+U_摩展开的新方法。
        """
        key = name.lower().strip()
        
        # 缓存
        if key in cls._KEYWORD_CACHE:
            return cls._KEYWORD_CACHE[key].copy()
        ...
        # 1. 精确关键词匹配
        for kw, entries in cls._SEMANTIC_HEX_MAP.items():
            if kw in key:
        ...
        cls._KEYWORD_CACHE[key] = probs.copy()
        return probs
    
    @classmethod
    def clear_cache(cls):
        cls._KEYWORD_CACHE.clear()
    
    def to_hex_state_semantic(self, name: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        高维语义版: 名称→64卦概率 → 概率幅
        
        Returns:
            (psi_64, psi_upper, psi_lower)
        """
        probs64 = self.encode_semantic_64(name)
        
        # 概率幅
        psi = np.sqrt(probs64 + 1e-12).astype(complex)
        psi /= np.linalg.norm(psi)
        
        return psi, None, None  # 不分解上下卦（直接64维）
